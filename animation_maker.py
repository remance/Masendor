...
def listscroll(scroll, listbox, currentrow, namelist, namegroup, uiclass, layer=19, oldlist=None):
    if mouse_scrollup:
        currentrow -= 1
        if currentrow < 0:
            currentrow = 0
...
def reload_animation(animation, char):
    ...
    for index, selector in enumerate([p1_eye_selector, p1_mouth_selector, p2_eye_selector, p2_mouth_selector]):
        ...
        if face[index] not in (0, 1):
            thistext = face[index]
        selector.change_name(headtext[index] + str(thistext))
...
class Button(pygame.sprite.Sprite):
    ...
    def __init__(self, text, image, pos, fontsize=20):
        ...
        self.text = text
        self.pos = pos
        textsurface = self.font.render(str(text), True, (0, 0, 0))
    ...
    def change_text(self, text):
        if text != self.text:
            self.image = self.image_original.copy()
            self.text = text
            textsurface = self.font.render(self.text.capitalize(), True, (0, 0, 0))
            textrect = textsurface.get_rect(center=(int(self.image.get_width() / 2), int(self.image.get_height() / 2)))
            self.image.blit(textsurface, textrect)
            self.rect = self.image.get_rect(center=self.pos)
    ...
    def __init__(self, size, pos, type, part_images):
        ...
        if self.type in ("p1", "p2"):
            ...
            for boxpart in ("W1", "W2"):
                textsurface = self.boxfont.render(boxpart, True, (0, 0, 0))
        ...
        if specific_part is not None:
            ...
            if check_mouse_pos is not None:
                for index, rect in enumerate(self.rect_part_list):
                    ...
                    if "special" not in rect and thisrect is not None and thisrect.collidepoint(check_mouse_pos):
                        click_any = True
                        if shift_press:
        ...
        for index, part in enumerate(self.rect_part_list.keys()):
            if self.stat2 is not None and part in self.stat2 and self.stat1[part] is not None and self.stat2[part] is not None:
                ...
                if part in self.part_selected:  # green text for selected part
                    textcolour = (20, 90, 20)
                textsurface1 = self.font.render(stat1, True, textcolour)

                textsurface2 = self.font.render(stat2, True, textcolour)
...
class Skeleton:
        ...
        if name is not None:
            ...
            for index, pose in enumerate(frame_list):
                ...
                for part in part_name_header:
                    if part in pose and pose[part] != [0] and any(ext in part for ext in exceptlist) is False:
                        if "weapon" in part:
    ...
    def grab_facepart(self, race, side, part, part_check, part_default):
    ...
    def generate_body(self, bodypart_list):
        ...
        try:
            ...
            p1_face = [gen_body_sprite_pool[p1_head_race][p1_head_side]["eyebrow"][self.p1_eyebrow].copy(),
                       self.grab_facepart(p1_head_race, p1_head_side, "eye", bodypart_list["p1_eye"], self.p1_any_eye),
                       gen_body_sprite_pool[p1_head_race][p1_head_side]["beard"][self.p1_beard].copy(),
                       self.grab_facepart(p1_head_race, p1_head_side, "mouth", bodypart_list["p1_mouth"], self.p1_any_mouth)]
            # if skin != "white":
        ...
        try:
            ...
            p2_face = [gen_body_sprite_pool[p2_head_race][p2_head_side]["eyebrow"][self.p2_eyebrow].copy(),
                       self.grab_facepart(p2_head_race, p2_head_side, "eye", bodypart_list["p2_eye"], self.p2_any_eye),
                       gen_body_sprite_pool[p2_head_race][p2_head_side]["beard"][self.p2_beard].copy(),
                       self.grab_facepart(p2_head_race, p2_head_side, "mouth", bodypart_list["p2_mouth"], self.p2_any_mouth)]
            # if skin != "white":
            #     face[0] = self.apply_colour(face[0], skin_colour)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 2:  # left click
                ...
                if popup_listbox in ui and popup_listbox.rect.collidepoint(mouse_pos):
                    currentpopuprow = listscroll(popup_listscroll, popup_listbox, currentpopuprow, popup_listbox.namelist, popup_namegroup, ui)
                    ...
                    current_anim_row = listscroll(anim_prop_listscroll, anim_prop_listbox, current_anim_row, anim_prop_listbox.namelist, anim_prop_namegroup, ui,
                    ...
                    current_frame_row = listscroll(frame_prop_listscroll, frame_prop_listbox, current_frame_row, frame_prop_listbox.namelist[current_frame], frame_prop_namegroup, ui,
            ...
            if popup_click is False:
                if mouse_up:
                    if clear_button.rect.collidepoint(mouse_pos):
                        ...
                        popuplist_newopen("p2_eye_select",
                        ...
                        popuplist_newopen("p2_mouth_select",
                        ...
                        helper_click = False
                        for index, helper in enumerate(helperlist):  # click on helper
                ...
                if showroom.rect.collidepoint(mouse_pos):  # mouse at show room
                    ...
                    if mouse_up:  # left click on showroom

...
class TroopAnimationData:
    def __init__(self, data_dir, module_dir, race_list, team_colour):
        ...
        with open(os.path.join(module_dir, "sprite", "subunit", "weapon", "joint.csv"), encoding="utf-8",
                  mode="r") as edit_file:
            rd = tuple(csv.reader(edit_file, quoting=csv.QUOTE_ALL))
            header = rd[0]
            ...
            try:
                ...
                for folder in subdirectories:
                    if folder[1] != "armour":
                        ...
                        part_subfolder = Path(os.path.join(module_dir, "sprite", "subunit", race, folder[-1]))
                        ...
                        for subfolder in subsubdirectories:
                            imgs = load_images(module_dir, subfolder=subfolder)
        ...
        for race in race_list:
            ...
            try:
                part_subfolder = Path(
                    os.path.join(module_dir, "sprite", "subunit", race, "armour"))
                subdirectories = [os.path.split(os.sep.join(
                    os.path.normpath(x).split(os.sep)[os.path.normpath(x).split(os.sep).index("sprite"):])) for x
                    in part_subfolder.iterdir() if x.is_dir()]
        ...
        for folder in subdirectories:
            ...
            for subfolder in subsubdirectories:
                ...
                icon_imgs = load_images(module_dir, subfolder=("sprite", "subunit", "weapon", folder[-1], subfolder[-1]))
                self.gen_weapon_sprite_pool[folder[-1]][subfolder[-1]]["icon"] = icon_imgs
                imgs = load_images(module_dir, subfolder=("sprite", "subunit", "weapon",

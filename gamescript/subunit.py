...
from gamescript.tactical import script_other, script_battle, script_subunit
...
class Subunit(pygame.sprite.Sprite):
    ...
    dmg_cal = script_battle.dmg_cal
    weapon_list = None
    armour_list = None
    stat_list = None
    set_rotate = script_other.set_rotate
    change_leader = script_subunit.change_leader
    ...
    def __init__(self, troop_id, game_id, unit, start_pos, start_hp, start_stamina, unit_scale):
        ...
        if len(self.trait) > 0:
            ...
            self.add_trait(stat, start_hp, start_stamina, unit_scale)
        ...
        if self.state != 100:  # only run these when not dead
            ...
            if dt > 0:  # only run these when self not pause
                ...
                if self.angle != self.new_angle:
                    ...
                    rotate_tiny = self.rotatespeed * dt  # rotate little by little according to time
                ...
                if (self.base_pos != self.base_target or self.charge_momentum > 1) and \
                    ...
                    if self.stamina > 0 and nocolide_check and enemycollide_check is False and \
                        ...
                        if self.chargeskill in self.skill_effect and self.base_pos == self.base_target and parent_state == 10:

...
def add_weapon_stat(self):
    ...
    for index, weapon in enumerate([self.primary_main_weapon, self.primary_sub_weapon, self.secondary_main_weapon, self.secondary_sub_weapon]):
        ...
        if self.weapon_data.weapon_list[weapon[0]]["Magazine"]:  # melee weapon if no ammo
            ...
            self.melee_penetrate[index] = self.weapon_data.weapon_list[weapon[0]]["Armour Penetration"] * \
                                          self.weapon_data.quality[weapon[1]]
        else:
            self.range_dmg[index][0] = self.weapon_data.weapon_list[weapon[0]]["Minimum Damage"] * \
                                       self.weapon_data.quality[weapon[1]]
            self.range_dmg[index][1] = self.weapon_data.weapon_list[weapon[0]]["Maximum Damage"] * \
                                       self.weapon_data.quality[weapon[1]]
            ...
            self.arrow_speed[index] = self.weapon_data.weapon_list[weapon[0]]["Travel Speed"]  # travel speed of range melee_attack

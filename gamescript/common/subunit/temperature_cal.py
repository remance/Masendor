def temperature_cal(self, temp_reach):
    ...
    if temp_reach < 0:  # cold temperature
        temp_reach = temp_reach * (100 - self.cold_resistance) / 100  # lowest temperature the subunit will change based on cold resist
    else:  # hot temperature
        temp_reach = temp_reach * (100 - self.heat_resistance) / 100  # highest temperature the subunit will change based on heat resist

    if self.temperature_count < temp_reach:
        self.temperature_count += (101 - self.heat_resistance) / 100 * self.timer  # increase temperature, rate depends on heat resistance
    elif self.temperature_count > temp_reach:
        self.temperature_count -= (101 - self.cold_resistance) / 100 * self.timer  # decrease temperature, rate depends on cold resistance

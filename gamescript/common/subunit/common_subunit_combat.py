...
def loss_cal(attacker, receiver, dmg, morale_dmg, leader_dmg, dmg_effect, timer_mod):
    ...
    if final_dmg > receiver.subunit_health:  # dmg cannot be higher than remaining health
        final_dmg = receiver.subunit_health

    receiver.subunit_health -= final_dmg
    health_check = 0.1
    if receiver.max_health != infinity:
        health_check = 1 - (receiver.subunit_health / receiver.max_health)
...
def die(self):
    ...
    self.dead_subunit_leader_logic("Destroyed")

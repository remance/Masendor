...
def skill_convert(self, skill_list, add_charge_skill=False):
    ...
    skill_dict = {skill: skill_dict[skill] for skill in skill_dict if skill == 0 or   # remove skill if class mismatch
...
def process_trait_skill(self):
    ...
    for weapon_set in self.weapon_skill:
        for weapon in self.weapon_skill[weapon_set]:
            if self.weapon_skill[weapon_set][weapon] not in self.skill:
                self.weapon_skill[weapon_set][weapon] = 0

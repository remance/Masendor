    ...
    if 0 not in map_feature_mod[
        ...
        if 1 in map_feature_mod["Status"]:
            if "Wet" in self.status_list:  # Shallow water type terrain
                self.apply_effect(self.status_list["Wet"], self.status_list, self.status_effect)
        ...
        if 2 in map_feature_mod["Status"]:  # Rot type terrain
            if "Decay" in self.status_list:
                self.apply_effect(self.status_list["Decay"], self.status_list, self.status_effect)

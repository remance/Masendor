    ...
    if self.alive and self.not_broken:
        ...
        if "uncontrollable" not in self.current_action and "uncontrollable" not in self.command_action:
            if self.battle.player_char_input_delay == 0 and not self.current_action:  # for input that need to have time delay to work properly
                if self.battle.player_keyboard_press["Swap Weapon Set 1"] and self.equipped_weapon != 0:  # Swap to primary weapon
                    self.swap_weapon(0)
                    self.battle.player_char_input_delay = 1
                elif self.battle.player_keyboard_press["Swap Weapon Set 2"] and self.equipped_weapon != 1:  # Swap to secondary weapon
                    self.swap_weapon(1)
                    self.battle.player_char_input_delay = 1
            ...
            if not self.move_speed:  # attack while stationary
                if self.battle.player_keyboard_press["Main Weapon Attack"] or self.battle.player_keyboard_press["Sub Weapon Attack"]:
                    action_num = 0
                ...
                elif self.battle.player_keyboard_hold["Main Weapon Attack"] or self.battle.player_keyboard_hold["Sub Weapon Attack"]:
                    action_num = 0
                    str_action_num = "0"
                ...
                if self.battle.player_keyboard_press["Main Weapon Attack"] or self.battle.player_keyboard_press["Sub Weapon Attack"]:  # perform attack
                    action_num = 0
                    if self.battle.player_keyboard_press["Sub Weapon Attack"]:
                        action_num = 1

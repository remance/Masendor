...
def make_editor_ui(main_dir, screen_scale, screen_rect, listbox_image, image_list, scale_ui, colour, updater):
    ...
    preset_select_border = uniteditor.SelectedPresetBorder(
    ...
    terrain_change_button = menu.TextBox(screen_scale, box_image.copy(),
                                         (screen_rect.width / 3, screen_rect.height - box_image.get_height()),
                                         "Temperate")  # start with temperate terrain
    feature_change_button = menu.TextBox(screen_scale, box_image.copy(),
                                         (screen_rect.width / 2, screen_rect.height - box_image.get_height()),
                                         "Plain")  # start with plain feature
    weather_change_button = menu.TextBox(screen_scale, box_image.copy(),
                                         (screen_rect.width / 1.5, screen_rect.height - box_image.get_height()),
    ...
    image1 = load_image(main_dir, screen_scale, "team1_button.png",
                        ("ui", "mainmenu_ui"))  # change unit slot to team 1 in editor
    image2 = load_image(main_dir, screen_scale, "team2_button.png",
                        ("ui", "mainmenu_ui"))  # change unit slot to team 2 in editor
    ...
    return {"unit_listbox": unit_preset_listbox, "preset_select_border": preset_select_border,
            ...
            "weather_change_button": weather_change_button, "filter_box": filter_box,
            "team_change_button": team_change_button,

    ...
    for subunit_id, this_subunit in who_todo.items():
        ...
        if subunit_id not in (0, "h1") and this_subunit["Sprite ID"] != "":  # skip None troop
            ...
            if preview:  # only create random right side sprite for preview in lorebook
                ...
                animation_list = [this_animation for this_animation in animation_list if "Preview" not in
                                  this_animation and race in this_animation and
                                  ((mount_race_name == "None" and "&" not in this_animation) or
                                   mount_race_name in this_animation) and
                    ...
                    for weapon_index, weapon in enumerate(weapon_set):
                        ...
                        for this_animation in reversed(temp_list): # check if weapon common type for same animation exist
                            if "_any_" in this_animation and any(weapon_common_action[weapon_set_index][weapon_index] in
                                                                 animation for animation in temp_list):
                    ...
                    for weapon_set_index, weapon_set in enumerate(
                        ...
                        for weapon_index, weapon in enumerate(weapon_set):
                            ...
                            if "_any_" in name_input:  # replace any type to weapon action name for reading later
                                name_input = name_input.replace("_any_", weapon_common_action[weapon_set_index][weapon_index])

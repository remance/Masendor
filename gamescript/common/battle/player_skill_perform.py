def player_skill_perform(self):
    ...
    if self.player_char.base_pos.distance_to(self.command_cursor_pos) <= skill_range:
        can_shoot = True
        if self.player_char.shoot_line not in self.battle_camera:
            self.battle_camera.add(self.player_char.shoot_line)
        self.player_char.shoot_line.update(self.command_cursor_pos, self.base_cursor_pos, can_shoot)
    else:
        can_shoot = False

...
def check_split(self):
    ...
    if np.array_split(self.subunit_id_array, 2, axis=1)[0].size >= 10 and np.array_split(self.subunit_id_array, 2, axis=1)[1].size >= 10 and \
    ...
    # Split by middle row
    if np.array_split(self.subunit_id_array, 2)[0].size >= 10 and np.array_split(self.subunit_id_array, 2)[1].size >= 10 and \
            self.leader[1].name != "None":

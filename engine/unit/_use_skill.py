...
from engine import effectsprite
    ...
    if which_skill in self.skill:
        ...
        if which_skill != self.charge_skill and "weapon" not in self.current_action:
            ...
            if skill_stat["Effect Sprite"]:
                effectsprite.Effect(self, base_pos, pos, pos, skill_stat["Effect Sprite"][0],
            ...
            for this_unit in group:
                ...
                if aoe >= distance:
                    ...
                    if "Action" in skill_stat["Type"]:  # effect that is use only when unit perform attack
                        if "Melee" in skill_stat["Type"] and which_skill not in this_unit[0].active_action_skill["melee"]:

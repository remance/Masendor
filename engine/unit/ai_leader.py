...
def ai_leader(self):
    ...
    if self.tactic == "Attack" and self.nearest_enemy:
        if "range" in self.group_type or "artillery" in self.group_type:
            ...
            if self.alive_troop_follower and self.group_formation_position != "Behind":
                # change formation so troop is behind leader for ranged group
            ...
            if enemy_distance > distance:
                if self.alive_troop_follower and self.group_formation_phase != "Skirmish Phase":  # start skirmish phase to maximise shooting
                    self.setup_formation("group", phase="Skirmish Phase")
                if self.alive_troop_follower and self.army_formation_phase != "Skirmish Phase":
            ...
            if "inf" in self.group_type:
                # group make up ranged troop, go to position that all troop followers can shoot
                if self.group_formation_position != "Ahead":  # change formation so troop is behind leader for ranged group
                    self.setup_formation("group", position="Ahead")

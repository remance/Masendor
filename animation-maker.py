...
Game.art_style_dir = art_style_dir
...
frame_property_list = ["hold", "p1_weapon1_l_hand", "p2_weapon1_l_hand",
                       ...
                       "p1_fixed_weapon1_pos", "p1_fixed_weapon2_pos", "p2_fixed_weapon1_pos", "p2_fixed_weapon2_pos",
                       "p3_fixed_weapon1_pos", "p3_fixed_weapon2_pos", "p4_fixed_weapon1_pos", "p4_fixed_weapon2_pos",
...
class BodyHelper(pygame.sprite.Sprite):
    def __init__(self, size, pos, ui_type, part_images):
        self._layer = 6
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.font_size = int(10 * screen_scale[1])
...
class Model:
    def __init__(self):
        ...
        for p in range(1, max_person + 1):
            ...
            self.all_part_list = self.all_part_list | {"p" + str(p) + "_head": None, "p" + str(p) + "_eye": "",
                                                       "p" + str(p) + "_mouth": "",
                                                       "p" + str(p) + "_neck": None,
    ...
    def read_animation(self, name, old=False, new_size=True):
        ...
        if name is not None:
            ...
            for index, pose in enumerate(frame_list):
                ...
                for part in pose:
                    if pose[part] and "property" not in part and part != "size":
    ...
    def edit_part(self, edit_mouse_pos, edit_type, specific_frame=None):
        ...
        #     self.part_selected = []
            ...
            if "Any" in edit_type:
                self.bodypart_list[edit_frame][edit_type[0:2] + "_mouth"] = ""
            else:
            ...
            if self.part_selected:
                ...
                if not self.animation_part_list[edit_frame][part_index]:
                    self.animation_part_list[edit_frame][part_index] = [0, pygame.Vector2(0, 0), [50, 50], 0, 0, 0, 1]
            ...
            p_face = {}
            for p in range(1, max_person + 1):
                p_face = p_face | {"p" + str(p) + "_eye": "", "p" + str(p) + "_mouth": ""}
            self.bodypart_list = [{key: value for key, value in self.all_part_list.items()}] * max_frame
            ...
            if not specific_frame or (frame_num == specific_frame):
                ...
                for key in self.mask_part_list:
                    try:
                        ...
                        self.frame_list[frame_num][key] = []
                for key in self.frame_list[frame_num]:
                    if "weapon" in key and self.frame_list[frame_num][key]:
                        self.frame_list[frame_num][key] = self.frame_list[frame_num][key][1:]
                p_face = {}
                for p in p_list:
                    p_face = p_face | {p: {p + "_eye": self.bodypart_list[frame_num][p + "_eye"],
                                           p + "_mouth": self.bodypart_list[frame_num][p + "_mouth"]}}
                for p in p_face:
                    p_face_pos = list(self.frame_list[frame_num].keys()).index(p + "_head") + 1
                    self.frame_list[frame_num] = {k: v for k, v in (
                                list(self.frame_list[frame_num].items())[:p_face_pos] + list(p_face[p].items()) +
                                list(self.frame_list[frame_num].items())[p_face_pos:])}

                self.frame_list[frame_num]["size"] = self.size
...
showroom_scale = (500, 500)
showroom_scale_mul = (default_sprite_size[0] / showroom_scale[0], default_sprite_size[1] / showroom_scale[1])
showroom = showroom.Showroom(showroom_scale, screen_size)
ui.add(showroom)
...
joint_button = SwitchButton(["Joint:OFF", "Joint:ON"], image,
                            (screen_size[0] - (image.get_width() * 0.5),
                             p_body_helper.rect.midtop[1] - (image.get_height() * 5)),
                            description=("Show part joints", "Display or hide part's joint icon."))
grid_button = SwitchButton(["Grid:ON", "Grid:OFF"], image,
                           (screen_size[0] - (image.get_width() * 1.5),
                            p_body_helper.rect.midtop[1] - (image.get_height() * 4)),
...
reset_button = Button("Reset", image, (screen_size[0] / 1.3, p_body_helper.rect.midtop[1] - (image.get_height() / 1.5)),
                      description=("Reset part edit", "Reset angle and flip."))
weapon_joint_to_hand_button = Button("W.J.T.H", image, (screen_size[0] / 1.3,
                                                        p_body_helper.rect.midtop[1] - (image.get_height() * 5)),
